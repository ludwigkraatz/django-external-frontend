...
class VirtualSourceExtension(Extension):
    def handle_import(self, name, compilation, rule):
        for source_key, source in compilation.source_index.items():
            source_name = source_key[1]
            if (source_name == name) or(
                source_name == '_' + name
            ) or (
                source_name == name + '.scss'
            ) or (
                source_name == name + '.sass'
            ) or (
                source_name == name + '.css'
            )or (
                source_name == '_' + name + '.scss'
            ) or (
                source_name == '_' + name + '.sass'
            ) or (
                source_name == '_' + name + '.css'
            ):
                return compilation.source_index[source_key]
        return None
...
class Scss(object):
    ...
    replacement.
    """
    __credits__ = "https://github.com/Kronuz/pyScss/blob/master/scss/legacy.py"
    ...
    def compile(
        ...
        self.root_namespace = root_namespace = Namespace(
